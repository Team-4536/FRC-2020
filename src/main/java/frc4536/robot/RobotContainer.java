...
public class RobotContainer {
  ...
  public final DriveTrain m_driveTrain = new DriveTrain(
      m_robotHardware.getDrivetrainLeftMotor(),
      m_robotHardware.getDrivetrainRightMotor(),
      m_robotHardware.getDrivetrainNavX(), m_robotHardware.getConstants());
  public final Shooter m_shooter =
      new Shooter(m_robotHardware.getTopShooterFlywheelMotor(),
                  m_robotHardware.getBottomShooterFlywheelMotor());
  public final Conveyor m_conveyor = new Conveyor(
      m_robotHardware.getBeltMotor(), m_robotHardware.getConveyorBlocker(),
      m_robotHardware.getConveyorBeam());
  public final Intake m_intake = new Intake(
      m_robotHardware.getIntakeMotor(), m_robotHardware.getIntakeExtender());
  public final Climber m_climber = new Climber(
      m_robotHardware.getClimberArmMotor(), m_robotHardware.getLiftMotor(),
      m_robotHardware.getBottomLimitSwitch());
  ...
  Trajectory t_startToShoot = TrajectoryGenerator.generateTrajectory(
      Poses.TRENCH_START, new ArrayList<>(), Poses.AUTO_TRENCH_SHOOT,
      m_driveTrain.getConfig().setReversed(false));
  Trajectory t_shootToEnd = TrajectoryGenerator.generateTrajectory(
      Poses.AUTO_TRENCH_SHOOT, new ArrayList<>(), Poses.TRENCH_END,
      m_driveTrain.getConfig().setReversed(false));
  Trajectory t_endToShoot = TrajectoryGenerator.generateTrajectory(
      Poses.TRENCH_END, new ArrayList<>(), Poses.AUTO_TRENCH_SHOOT,
      m_driveTrain.getConfig().setReversed(true));

  Trajectory t_toRendezShoot = TrajectoryGenerator.generateTrajectory(
      Poses.TRENCH_START, new ArrayList<>(), Poses.RENDEZ_SHOOT,
      m_driveTrain.getConfig().setReversed(false));
  Trajectory t_shootToRendez = TrajectoryGenerator.generateTrajectory(
      Poses.RENDEZ_SHOOT, new ArrayList<>(), Poses.RENDEZ_SWEEP,
      m_driveTrain.getConfig().setReversed(false));
  Trajectory t_rendezToShoot = TrajectoryGenerator.generateTrajectory(
      Poses.RENDEZ_SWEEP, new ArrayList<>(), Poses.RENDEZ_SHOOT,
      m_driveTrain.getConfig().setReversed(true));
  Trajectory t_centerAuto = TrajectoryGenerator.generateTrajectory(
      Poses.CENTER_AUTO_START, new ArrayList<>(), Poses.CENTER_AUTO_END,
      m_driveTrain.getConfig().setReversed(false));
  ...
  private void configureButtonBindings() {
    ...
    new JoystickButton(m_driveController, Button.kBumperRight.value)
        .whileHeld(new IntakeCommands(m_intake, m_conveyor,
                                      m_driveController::getAButton)); // Intake
    ...
    new JoystickButton(m_driveController,
                       Button.kY.value) // Spin up shooter and automatically
                                        // fires when shooter reaches a speed.
                                            .whileHeld(new ShootCommand(
                                                m_shooter, m_conveyor));
    new JoystickButton(m_driveController,
                       Button.kX.value) // reverse move conveyor
        .whileHeld(
            new RunCommand(()
                               -> m_conveyor.moveConveyor(
                                   -Constants.CONVEYOR_INTAKE_SPEED, true),
                           m_conveyor));
    ...
    new JoystickButton(m_operatorJoystick, 7) // Conveyor manual control
        .whileHeld(new RunCommand(
            ()
                -> m_conveyor.moveConveyor(-m_operatorJoystick.getY(), true),
            m_conveyor));
    new JoystickButton(m_operatorJoystick, 8) // Intake manual control
        .whileHeld(new RunCommand(
            () -> m_intake.intake(-m_operatorJoystick.getY()), m_intake));
    new JoystickButton(m_operatorJoystick, 2) // Spinup shooter
        .whileHeld(new ManualShooter(m_operatorJoystick::getX, m_driveTrain,
                                     m_shooter));
  ...
  private void configureDefaultCommands() {
    ...
    CommandBase default_driveTrain = new RunCommand(() -> {
      ...
      m_driveTrain.arcadeDrive(
          (button ? 0.4 : (trigger ? 0.6 : 1.0)) *
              deadzone(m_driveController.getY(GenericHID.Hand.kLeft),
                       Constants.DRIVE_DEADZONE),
          (button ? 0.4 : (trigger ? 0.6 : 0.9)) *
              deadzone(m_driveController.getX(GenericHID.Hand.kRight),
                       Constants.DRIVE_DEADZONE),
          true);
    }, m_driveTrain);
    CommandBase default_climber = new RunCommand(() -> { // climber
      m_climber.setWinch(m_operatorJoystick.getRawButton(3)
                             ? -1
                             : (m_operatorJoystick.getRawButton(4)
                                    ? -m_operatorJoystick.getY()
                                    : 0));
      m_climber.setArm(
          m_operatorJoystick.getRawButton(5) ? -m_operatorJoystick.getY() : 0);
    }, m_climber);
    ...
    CommandBase default_shooter = new RunCommand(() -> {         // shooter
      if (m_driveController.getTriggerAxis(Hand.kRight) > 0.5) { // change to
                                                                 // 0.5
  ...
  public Command generateAutoCommands(Autonomous chose, Pose2d initialPose) {
    ...
    case TRENCH:
      return new TrenchAutoCommand(m_shooter, m_conveyor, m_driveTrain,
                                   m_intake, initialPose, t_shootToEnd,
                                   t_endToShoot);
    case VISION_TEST:
      return new VisionTestAutoCommand(m_shooter, m_conveyor, m_driveTrain,
                                       m_intake, t_startToShoot);
    case RENDEZVOUS:
      return new RendezvousAutoCommand(m_shooter, m_conveyor, m_driveTrain,
                                       m_intake, initialPose, t_shootToRendez,
                                       t_rendezToShoot);
    case CENTER_AUTO:
      return new CenterAutoCommand(m_shooter, m_conveyor, m_driveTrain,
                                   initialPose);
    case BASELINE:
      return new RunCommand(
                 () -> m_driveTrain.arcadeDrive(-0.3, 0), m_driveTrain)
          .withTimeout(1)
          .andThen(new RunCommand(
              () -> m_driveTrain.arcadeDrive(0, 0), m_driveTrain));
    default:
      return new CenterAutoCommand(m_shooter, m_conveyor, m_driveTrain,
                                   initialPose); // safety purposes lol
    }
  ...
  public Command getAutonomousCommand() {
    ...
    Pose2d initialPose =
        new Pose2d(m_xInitial.getDouble(0.0), m_yInitial.getDouble(0.0),
